...
public class Formula
{
    ...
    public Formula(string formula, Func<string, string> normalize, Func<string, bool> isValid)
    {
        ...
        foreach (string token in Formula.GetTokens(formula))
        {
            ...
            {
                throw new FormulaFormatException($"'{token}' is invalid infix syntax. Please reformat or delete it.");
            }
        }
        ...
        if (IsVariable(token))
        {
            ...
            if (!validator(str))
            {
                throw new FormulaFormatException
                    ($"The valid variable '{token}' has been normalized to '{str}', which is invalid syntax." +
                    $"Consider using a different token or normalizer.");
            }
    ...
    private static void VerifyFormula(List<string> tokens)
    {
        ...
        if (tokens.Count < 1)
        {
            throw new FormulaFormatException("Cannot parse empty formula. Please add at least one expression.");
        }
        ...
        {
            throw new FormulaFormatException($"Formula has a naked leading operator, {oper}. Remove it or add a leading value or variable.");
        }

        if (IsOperator(tokens[^1], out oper) && oper != ")")
        {
            throw new FormulaFormatException($"Formula has a naked trailing operator, {oper}. Remove it or add an ending value or variable.");
        }
    ...
    private static void VerifyParenRules(List<string> tokens)
    {
        ...
        foreach (string token in tokens)
        {
            ...
            {
                throw new FormulaFormatException
                    (String.Format($"Formula contains too many closing parentheses at closing parenthesis # {rightParens}. " +
                    $"Consider deleting or adding left parentheses."));
            }
        }
        ...
        {
            throw new FormulaFormatException
                (String.Format($"Formula contains {0} unclosed parentheses. Consider closing or removing left parentheses.",
    ...
    private static void VerifyFollowingRules(List<string> tokens)
    {
        ...
        foreach (string token in GetNonParenTokens(tokens))
        {
            ...
            {
                throw new FormulaFormatException
                    ($"Consecutive operators, {token} and {previousToken}, detected. Please delete one or insert a value or variable.");
            }
            ...
            {
                throw new FormulaFormatException
                        ($"Consecutive values, {token} and {previousToken}, detected. Please delete one or insert an operator.");
            }
    ...
    public object Evaluate(Func<string, double> lookup)
    {
        ...
        foreach (string token in tokens)
        {
            ...
            else if (IsVariable(token))
            {
                ...
                {
                    return new FormulaError($"Failed get to get lookup value for variable {token}. " +
        ...
        {
            case double.NaN:
                return new FormulaError($"Formula resulted in 0 divided by 0. " +
                    "Please check expression and variable values.");
            case double.PositiveInfinity:
                return new FormulaError($"Formula resulted in positive division by 0. " +
                    "Please check expression and variable values.");
            case double.NegativeInfinity:
                return new FormulaError($"Formula resulted in negative division by 0. " +
                    "Please check expression and variable values.");
            default: break;
        }
